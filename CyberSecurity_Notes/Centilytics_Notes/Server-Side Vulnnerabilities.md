Link : https://portswigger.net/web-security/learning-paths/server-side-vulnerabilities-apprentice
...
Solution : Simply check the `robots.txt` and then get onto the `/administrator-panel` and delete the account for Carlos
...
Solution : In the home page only, there is a (index.js) which contained some URL like `/admin-4x15eh` that will be visited only if the user is the admin. I simply visited the website and deleted `carlos` 's account.
...
Solution : We can see that there is a cookie parameter as `Admin=false`, so just visit the `/admin`  with the cookie parameter set to True, i.e. `Admin=true`. And then we can delete the user `carlos` simply by clicking onto the link.
...
Solution: I logged on to the credentials provided and then saw that in the each post, the name of the author of the post contained the GUID of the author, so I found the post made by `carlos` and got his `GUID` and after getting that went to the packet captured for the `login` page and replaced the `GUID` of `weiner` to that of the `carlos` and hence I was able to get the API Key of `carlos`.
...
Solution: First I was required to brute-force the usernames to get a correct username. And then I got the correct usernames as `archie` and then using a simple Sniper Attack, I cracked the password as `nicole`.
...
Solution: I first logged into the account of `winer` and saw that the email client has the mail with the OTP. And I entered the OTP and noted the exact URL to get onto to my account. And then I bypassed the OTP page for `carlos` by just changing the ID.
...
Solution : First change the StockAPI URL to http://localhost/admin and then copy the URL used to delete the account of `carlos` and then copy that URL and again put that in the StockAPI URL in Request.
...
Solution : I saw that there was IP address it was fetching details from, for the Stock Checking, I simply ran a Sniper Attack onto the last bit of the IP Address to get the admin functionalities, and it was `192.168.0.70:8080/admin`, where the functionality to delete `carlos` was present and then I copied the URL for deletion and pasted that onto the StockAPI Parameter and the lab got solved.
...
Solution : We need to provide the command in the Stock Checking functionality as `storeId=1|whoami`
...
Solution : I have just added `' OR 1=1 --` to the URL while it was filtering the products data.
...
Solution : Just put the username as `administrator' --` and this will comment an query working after it.