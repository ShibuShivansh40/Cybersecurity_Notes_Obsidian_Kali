...
**Command to search for all John Modules :** `locate *2john*`
...
[sudo] password for shibushivansh: 
Reading package lists... Done
Building dependency tree... Done
Reading state information... Done
dislocker is already the newest version (0.7.3+git20250907-1).
Solving dependencies... Done
0 upgraded, 0 newly installed, 0 to remove and 1788 not upgraded.
...
┌──(shibushivansh㉿shibu)-[~/Downloads]
└─$ sudo dislocker /dev/loop24p1 -u1234qwer -- /media/bitlocker
[sudo] password for shibushivansh: 
Tue Jan 27 10:46:07 2026 [CRITICAL] None of the provided decryption mean is decrypting the keys. Abort.
Tue Jan 27 10:46:07 2026 [CRITICAL] Unable to grab VMK or FVEK. Abort.

┌──(shibushivansh㉿shibu)-[~/Downloads]
└─$ cd /media/bitlockermount  

┌──(shibushivansh㉿shibu)-[/media/bitlockermount]
└─$ ls -la                                          
total 8
drwxr-xr-x 2 root root 4096 Jan 27 00:08 .
drwxr-xr-x 6 root root 4096 Jan 27 00:08 ..
                      
┌──(shibushivansh㉿shibu)-[/media/bitlockermount]
└─$ cd /media/bitlocker     
    
┌──(shibushivansh㉿shibu)-[/media/bitlocker]
└─$ ls    

┌──(shibushivansh㉿shibu)-[/media/bitlocker]
└─$ ls -la
total 8
drwxr-xr-x 2 root root 4096 Jan 27 00:07 .
drwxr-xr-x 6 root root 4096 Jan 27 00:08 ..
...
### Network Services
Apart from web applications, these services include (but are not limited to) `FTP`, `SMB`, `NFS`, `IMAP/POP3`, `SSH`, `MySQL/MSSQL`, `RDP`, `WinRM`, `VNC`, `Telnet`, `SMTP`, and `LDAP`.
...
If Hydra shows an error like `[ERROR] invalid reply from target smb://10.129.42.197:445/` then we most likely have an outdated version of THC-Hydra that cannot handle SMBv3 replies. To work around this problem, we can manually update and recompile `hydra` or use another very powerful tool, the [Metasploit framework](https://www.metasploit.com/).

```shell-session
ShibuShivansh@htb[/htb]$ msfconsole -q
...
```shell-session
ShibuShivansh@htb[/htb]$ hydra -C user_pass.list ssh://10.100.38.23
```
